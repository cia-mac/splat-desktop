# Diagram 1 Spec — "Frame → sampling grid → point clusters" (v1)
...
The single canonical image that explains PixelCluster in one glance and kills the three biggest misconceptions: "it's a filter," "it's AI," "one pixel = one point." It is the lead asset for the website "How it works" section, the GitHub README, decks, and social. It is also the static fallback / storyboard for the interactive ITD-1 (grid inspector).
...
- **Hero / brand variant:** the ciamac.com bird engine output. Naturally Caspian, recognizable as the PixelCluster look. Used where brand resonance matters (social, deck cover).
