# PixelCluster — Website Explainer Assembly Spec (v1)

Created 2026-05-29. Operator: Claude. **Spec only — no code, no site edits, no publish.** Target page: `ciamac.com/pixelcluster`.
...
- **S6 "Take it with you" (explicit block below the fold):** all platforms listed plainly:
  ...
  - Each links to the GitHub release asset URL (`.../releases/download/pixelcluster-v0.1.1/<file>`).
...
>
> PixelCluster is **a renderer** — not a filter, not AI, not Gaussian splatting, not a physics simulation.

S6 download notes (small, under the buttons):

> Free. macOS builds are unsigned — on first launch you may see a Gatekeeper warning; run `xattr -cr /Applications/PixelCluster.app`. An internet connection is needed on first launch (the renderer library loads from a CDN). Apple Silicon only; Windows 10+ x64.

(S7 optional lineage, one line: "PixelCluster has been shipping since April 2026 as ciafx. v0.1 is the version that names it.")
