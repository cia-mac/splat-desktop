# Pointfield v0.1 — Launch Release Plan (v2)

v2 (2026-05-29): PixelCluster to Pointfield text swap of `RELEASE_PLAN_v1.md` (kept). Product-name strings and `pixelcluster-v*` tag refs updated to Pointfield; substance unchanged (name was already canon-locked). Public/site/release surfaces still read PixelCluster until rename Phases 2 to 4 run.

Created 2026-05-28. Operator-maintained. Notion holds strategy; this is the working launch sheet.

## North star

ciamac.com is the home of Pointfield. The public should feel they **discovered a strange rendering system already running** — not landed on a product page. Browser interaction is primary; downloads are secondary; the App Store is the long arc.

Narrative lock: **reveal, not build.** "Pointfield already existed inside my tools; v0.1 is the version that calls itself by name." No iOS/visionOS promises in public copy yet (that track is real but unannounced).

## Current state (verified 2026-05-28)

- **GitHub releases live:** `pointfield-v0.1.0` + `pointfield-v0.1.1` (Latest). Mac (.dmg, Apple Silicon), Windows (.exe setup, .msi). Unsigned.
- **Web surface built (stage / local, not promoted):**
  ...
  - Homepage easter egg (click the bird's head) rewired to Pointfield + v0.1.1 downloads, opens `/pixelcluster`.
...
2. ★ In App Store Connect: create the app record, reserve the name "Pointfield", bundle ID (see note below).
...
8. ★ Upload via Transporter. Fill App Store Connect metadata: screenshots, description, category (Graphics & Design or Photo & Video), privacy nutrition label (Pointfield collects nothing — declare "no data collected"), age rating.
