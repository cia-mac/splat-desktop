# Diagram 2 Spec — "One sampled region becomes seven points" (v1.1)

v1.1 (2026-05-29): PixelCluster to Pointfield text swap of `diagram-2_one-region-seven-points_spec_v1.md` (kept). Spec substance unchanged; only product-name strings updated.
...
Isolate and explain the **core Pointfield mechanism**: one sampled region of a frame becomes, by default, **seven rendered points** that all share one sampled color, scattered slightly and varied in size. Diagram 1 shows the whole frame; Diagram 2 zooms into a single cell and shows exactly what happens there.
...
- **Kicker:** `POINTFIELD — THE CORE STEP` (parallel to Diagram 1's `HOW IT WORKS`).
