...
const INTERACTIVE = !(BENCH || Q.has('matched') || Q.has('render') || Q.has('eval'));
...
function syncUI() {
  ...
  document.getElementById('edge').value = U.uEdge.value;
}
function bindUI() {
...
(async () => {
  try {
    ...
    if (INTERACTIVE && Q.has('autoload')) {
      ...
      await post('autoload.json', JSON.stringify({ log: document.getElementById('log').textContent, live: document.getElementById('live').textContent, baked: baked.n }));
    }
